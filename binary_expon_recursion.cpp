...
#define mk make_pair
#define fornv(x,v) for(auto x :v) cout<< x << ' ' 
...
//struct DisjointSet{ v64 parent; v64 size; DisjointSet(ll maxSize) { parent.resize(maxSize+1); size.resize(maxSize+1); for (ll i = 1; i <= maxSize; i++) { parent[i] = i; size[i] = 1; } }
...
v64 primeFactors(ll n){v64 v;while (n % 2 == 0){v.pb(2); n = n / 2;}for (int i = 3; i <= sqrt(n); i = i + 2){ while (n % i == 0) { v.pb(i); n = n / i;}}if (n > 2) {  v.pb(n);}return v;}
ll strint(string str){ll len = 0;forn(i,str.size()){len = len * 10 + (str[i] - '0');}return len;}
power with mode 
...
//int dx[] = {-1, 0, 1, 0};
//int dy[] = {0, 1, 0, -1};
//useInside for loop of 4 iteratiom
//int newX = x + dx[i];
//int newY = y + dy[i];
